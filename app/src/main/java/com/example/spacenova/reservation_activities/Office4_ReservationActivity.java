...
public class Office4_ReservationActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private JSONArray office4_jsonArray;
    private JSONArray _office4_jsonArray;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        office4_jsonArray = new JSONArray();
        for (int i = 0; i < roomTimeArrayList.size(); i++) {
            office4_jsonArray.put(roomTimeArrayList.get(i).getJSONObject());
        }

        syncRoomStatusImgOnFirebase();
        Log.d(MainActivity.TAG, "온 크리에이트 당시 쉐어드에 저장된 강의실 정보 값 : " + roomData.getAll().toString());
        Log.d(MainActivity.TAG, "초기화된 제이슨 배열 값 : " + office4_jsonArray.toString());
        //처음에 초기화된 제이슨을 넣어준 적이 없다면, 초기화하고 그렇지 않다면 만들어진 제이슨을 쉐어드에 저장. 이 저장 값을 기반으로 진행
        if (!roomData.getBoolean("IS_" + whichDayToRoomDate.toUpperCase() +"_OFFICE4_INIT_BEFORE", false)) {
            initRoomDataInfo();
...
//                    Log.d("싱크룸 키, 차일드 ", childSnapshot.getKey() + childSnapshot);
                    for (DataSnapshot grandchildSnapshot : childSnapshot.getChildren()) {
                        ...
                        if (grandchildSnapshot.getKey().equals("4사무실")){
...
//                    }
    ...
    private void syncSharedPrefWithJsonArray(JSONArray _office4_jsonArray){
        SharedPreferences.Editor editor = roomData.edit();
        editor.putString(whichToPutToRoomData,_office4_jsonArray.toString());
        editor.apply();
    }
    ...
    private void editRoomData(final Button reserveButton, final TextView tv, final int index){
        ...
        //해당값변경후
        //풋
        _office4_jsonArray = stringToJSONArray();
        for (int n = 0; n < _office4_jsonArray.length(); n++) {
            if (n == index) { //몇번째 오픈되는 강의실인지가 인덱스넘버
                try {
                    //아이디, 메모 수정
                    JSONObject object = _office4_jsonArray.getJSONObject(n);
                    object.put("isOccupied", true);
                    object.put("usedById",MainActivity.loginedUid);
                    object.put("memo",value);
                    Log.d(MainActivity.TAG, "파싱 후 제이슨 : " + _office4_jsonArray);
                    //해당 제이슨 배열과 같이 쉐어드 역시 싱크를 맞춰준다.
                    syncSharedPrefWithJsonArray(_office4_jsonArray);
        ...
        mFirebaseDatabaseReference.child(ROOM_CHILD).child(whichDayToRoomDate).child("4사무실").child(time).setValue(new RoomFirebase(true, MainActivity.loginedUid, value));
...
//        SharedPreferences.Editor editor = userData.edit();
        ...
        infoarr[6] = dayInKorean+" 4사무실 " + time;
    ...
    private void buildDialog(final Button reserveButton, final TextView tv, final int index) {
        ...
        builder = new AlertDialog.Builder(Office4_ReservationActivity.this);
    ...
    private void changeRoomStatusWhenReservationChanged(int indextochange) {
        ...
        if (_office_jsonArray != null) {
            ...
            for (int n = 0; n < _office_jsonArray.length(); n++) {
                ...
                //사실 n==index 이조건은 한화면 내에서의 처리를 위해 만든 조건문인데
                //이 조건문 때문에 1->4사무실 19:00 19:00로 이동시에 즉석에서 예약중 전환이 안되는 문제
                //한번 나갓다 들어와야함.
...
//    }
    ...
    public void initRoomDataInfo() {
        ...
        editor.putString(whichToPutToRoomData, office4_jsonArray.toString());
        editor.putBoolean("IS_"+whichDayToRoomDate.toUpperCase()+"_OFFICE4_INIT_BEFORE", true);
        editor.apply();
    }
