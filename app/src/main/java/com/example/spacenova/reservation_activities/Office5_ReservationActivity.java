...
public class Office5_ReservationActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private JSONArray office5_jsonArray;
    private JSONArray _office5_jsonArray;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //액티비티 생성시 1사무실의 모든 사용 시간을 관리하는 arraylist초기화
        ...
        office5_jsonArray = new JSONArray();
        for (int i = 0; i < roomTimeArrayList.size(); i++) {
            office5_jsonArray.put(roomTimeArrayList.get(i).getJSONObject());
        }

        syncRoomStatusImgOnFirebase();
        Log.d(MainActivity.TAG, "온 크리에이트 당시 쉐어드에 저장된 강의실 정보 값 : " + roomData.getAll().toString());
        Log.d(MainActivity.TAG, "초기화된 제이슨 배열 값 : " + office5_jsonArray.toString());
        //처음에 초기화된 제이슨을 넣어준 적이 없다면, 초기화하고 그렇지 않다면 만들어진 제이슨을 쉐어드에 저장. 이 저장 값을 기반으로 진행
        if (!roomData.getBoolean("IS_" + whichDayToRoomDate.toUpperCase() +"_OFFICE5_INIT_BEFORE", false)) {
            initRoomDataInfo();
...
//                    Log.d("싱크룸 키, 차일드 ", childSnapshot.getKey() + childSnapshot);
                    for (DataSnapshot grandchildSnapshot : childSnapshot.getChildren()) {
                        ...
                        if (grandchildSnapshot.getKey().equals("5사무실")){
...
//                    }
    ...
    private void syncSharedPrefWithJsonArray(JSONArray _office5_jsonArray){
        SharedPreferences.Editor editor = roomData.edit();
        editor.putString(whichToPutToRoomData,_office5_jsonArray.toString());
        editor.apply();
    }
    ...
    private void editRoomData(final Button reserveButton, final TextView tv, final int index){
        ...
        //해당값변경후
        //풋
        _office5_jsonArray = stringToJSONArray();
        for (int n = 0; n < _office5_jsonArray.length(); n++) {
            if (n == index) { //몇번째 오픈되는 강의실인지가 인덱스넘버
                try {
                    //아이디, 메모 수정
                    JSONObject object = _office5_jsonArray.getJSONObject(n);
                    object.put("isOccupied", true);
                    object.put("usedById",MainActivity.loginedUid);
                    object.put("memo",value);
                    Log.d(MainActivity.TAG, "파싱 후 제이슨 : " + _office5_jsonArray);
                    //해당 제이슨 배열과 같이 쉐어드 역시 싱크를 맞춰준다.
                    syncSharedPrefWithJsonArray(_office5_jsonArray);
        ...
        mFirebaseDatabaseReference.child(ROOM_CHILD).child(whichDayToRoomDate).child("5사무실").child(time).setValue(new RoomFirebase(true, MainActivity.loginedUid, value));
...
//        SharedPreferences.Editor editor = userData.edit();
        ...
        infoarr[6] = dayInKorean+" 5사무실 " + time;
    ...
    private void buildDialog(final Button reserveButton, final TextView tv, final int index) {
        ...
        builder = new AlertDialog.Builder(Office5_ReservationActivity.this);
...
//                editUserData(index);
    ...
    private void changeRoomStatusWhenReservationChanged(int indextochange) {
        ...
        //반환할 자리 번호 파싱
        //쉐어드 값 수정하기
        //원래 어디 자리인지 정보가 넘어오게 되어있다.
        //어떤요일의, 어떤 사무실의 정보를 가져와서, 해당 데이터를 false 처리해준다.
...
//    private void changeRoomStatusWhenReservationChanged(){
//        int index = -1;
//        String _roomTimeBefore = getSharedPreferences("userLogined",MODE_PRIVATE).getString(MainActivity.loginedUid,"").split(",")[6].split(" ")[2];
//
//        if (_roomTimeBefore.equals("09:00")){
...
//                    tvArrayList.get(index).setBackgroundColor(Color.parseColor("#005eff"));
//                    buttonArrayList.get(index).setClickable(true);
//                    buttonArrayList.get(index).setText("하브루타 예약하시겠어요?");
//                    syncSharedPrefWithJsonArray(_office5_jsonArray);
//                } catch (JSONException e) {
//                    e.printStackTrace();
//                }
...
//    }
    ...
    public void initRoomDataInfo() {
        ...
        editor.putString(whichToPutToRoomData, office5_jsonArray.toString());
        editor.putBoolean("IS_"+whichDayToRoomDate.toUpperCase()+"_OFFICE5_INIT_BEFORE", true);
        editor.apply();
    }
